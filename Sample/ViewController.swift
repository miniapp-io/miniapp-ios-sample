...
class WalletBridgeProvider : BridgeProvider {
    ...
    func onWebViewDestroy(_ webView: WKWebView) {
        webView.configuration.userContentController.removeScriptMessageHandler(forName: TrustWeb3Provider.scriptHandlerName)
    }
    ...
    func inject(contentController: WKUserContentController) {
        ...
        if let messageHandler = self.messageHandler {
            contentController.removeScriptMessageHandler(forName: TrustWeb3Provider.scriptHandlerName)
...
//                .url("https://game.bt.qiku.net/games/fruittri?code=LuLT2PdXscbH3TOmTpX+LVnPClMHSZFa66v7ygw40CExOCQXfkUaBZzEeM+hoOtW&channel=telegram&lan=zh-hans")
                .url("https://miniappx.io/apps/10?startapp=aa&roomId=2222")
